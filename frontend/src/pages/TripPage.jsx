...
import { createDay, getDays, updateDay } from "../../api/days";
...
export default function TripPage() {
  ...
  //get the dates for a trip from the start and end date
  ...
  const handleSaveTrip = async (tripData) => {
    try {
      ...
      if (editingTrip) {
        ...
        savedTrip = { trips_id: editingTrip.trips_id };

        const oldStartDate = new Date(editingTrip.trip_start_date);
        const newStartDate = new Date(tripData.trip_start_date);

        if (oldStartDate.getTime() !== newStartDate.getTime()) {
          try {
            const days = await getDays(editingTrip.trips_id);
            ...
            for (let i = 0; i < sortedDays.length; i++) {
              ...
              await updateDay(editingTrip.trips_id, sortedDays[i].day_id, {
                day_date: newDate,
              });
              currentDate.setDate(currentDate.getDate() + 1);
            }
            toast.info("Trip days updated to match new start date!");
          } catch (err) {
            console.error("Failed to update trip days:", err);
            toast.error("Failed to update days to match the new trip start date.");
          }
        }
        ...
        const { trip_start_date, trip_end_date } = tripData;
        if (trip_start_date && trip_end_date) {
          const dayDates = generateDateRange(trip_start_date, trip_end_date);
          for (const date of dayDates) {
            try {
              await createDay(savedTrip.trips_id, { day_date: date });
            } catch (err) {
              console.error(`Failed to create day for ${date}:`, err);
            }
          }
        }
      }
              ...
              <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                >
                  ...
                  <div className="date-inputs">
                    <label className="date-input-label" htmlFor="startDate">Start Date:</label>
                    <input
                      name="startDate"
                      type="date"
                      defaultValue={
                    ...
                    {!editingTrip && (
                      <>
                        <label className="date-input-label" htmlFor="endDate">End Date:</label>
                        <input
                          name="endDate"
                          type="date"
                          defaultValue={
