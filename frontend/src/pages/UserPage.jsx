...
This page is just a test page so it is not linked to anywhere in the app, so to access it, manually change the URL to http://localhost:5173/user
*/
...
export default function UserPage() {
    ...
    //Need to store state information for modifying user info
    ...
    const sendRequest = async (endpoint, method, body = null) => {
        ...
        const response = await fetch(`http://localhost:3000/user/${endpoint}`, options);
  ...
  return (
    <div>
        ...
        <div>
            ...
            <div><button onClick={() => sendRequest("create", "POST", { userId: user.user_id, createUsername })}>Create</button></div>
        </div>
        {/* Modify first name */}
        <div>
            ...
            <div><button onClick={() => sendRequest("modify", "PUT", { userId: user.user_id, field: "first_name", value: firstname })}>Modify first name</button></div>
        </div>
        ...
        <div>
            ...
            <div><button onClick={() => sendRequest("modify", "PUT", { userId: user.user_id, field: "last_name", value: lastname })}>Modify last name</button></div>
        </div>
        ...
        <div>
            ...
            <div><button onClick={() => sendRequest("modify", "PUT", { userId: user.user_id, field: "username", value: modifyUsername })}>Modify username</button></div>
        </div>
        ...
        <div>
            ...
            <div><button onClick={() => sendRequest("modify", "PUT", { userId: user.user_id, field: "email", value: email })}>Modify email</button></div>
        </div>

        {/* Read user info*/}
        <div>
            <strong>Read User Info</strong>
            <div><button onClick={() => sendRequest("read", "GET")}>Read</button></div>
        ...
        <div>
            ...
            <div><button onClick={() => sendRequest("delete", "DELETE", { userId: user.user_id })}>Delete</button></div>
        </div>
    </div>
  );
}